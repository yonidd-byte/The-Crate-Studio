...
#### **Law IV — The Universal Bottom Device Chain**
Every plugin, MIDI generator, Rack, Modulator, and Macro panel lives in **one horizontal chain at the bottom of the screen**, scoped to the currently selected track. Devices can be **folded into minimal headers** (a single title bar strip) to reclaim vertical space. There is exactly one place in this DAW where "a thing that processes signal" lives. The user never asks "where is that?"
...
| **Expandable Lanes** | Automation lanes expand *beneath* clips without hiding the source material (Pro Tools style). |
...
- Lanes are **expandable, not modal.** The arrangement never rescales or jumps. Clips stay visible; the lane pushes down below them.
- Tension/curve editing happens **on the curve itself** — drag the midpoint handle. No curve-type dropdown, no properties inspector. Right-click the segment for exotic shapes (Wave/Pulse).

> **⚠ Existing debt (see Gap Analysis):** current implementation hardcodes automation to a single Volume parameter and passes `nullptr` for the `UndoManager`. The parameter dropdown and undo integration are **blocking prerequisites** before any further curve work.
...
The parts that *are* built (device I/O, VST3 hosting, curve automation with durable persistence, arrangement UI, Device Chain, mixer chrome) are genuinely solid, and the TE integration patterns learned (plugin window hosting, curve baking, modal-dialog pitfalls, `ValueTree`/`Edit` lifecycle management, JUCE `dynamic_cast`-and-private-inheritance gotchas, `Viewport` subclassing for scroll sync) transfer directly forward. The real remaining gap is still breadth, not durability or polish: Piano Roll, MPE, ARA2, Live Mode, the Crate Brain's actual intelligence layer (ontology scanning, semantic search, auto-tagging), and the scripting API are all still zero. That's the next several phases, not a rescue mission.
